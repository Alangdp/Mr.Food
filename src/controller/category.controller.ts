...
const store: RequestHandler = async (req, res) => {
  try {
    ...
    if(!name) return response(res, {errors: [{message: "Name is required"}], status: 400});
    let category = await Category.findByNameAndCompanyId(name, companyId);
    if(category) return response(res, {errors: [{message: "Category already exists", data: []}], status: 400});
...
const deleteCategory: RequestHandler = async (req, res) => {
  try {
    ...
    if(!category || (category && category.companyId !== compnayId)) return response(res, {errors: [{message: "Category not found"}], status: 404});
